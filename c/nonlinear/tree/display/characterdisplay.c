...
asciinode *build_ascii_tree_recursive(struct treenode *root)
{
    ...
    node->left = build_ascii_tree_recursive(root->lchild);
    node->right = build_ascii_tree_recursive(root->rchild);
    ...
    sprintf(node->label, "%c", root->payload);
...
void compute_edge_lengths(asciinode *node)
{
    ...
    else
    {
        ...
        for (i = 0; i < hmin; i++)
        {
            delta = MAX(delta, gapl + 1 + rprofile[i] - lprofile[i]);
        }
