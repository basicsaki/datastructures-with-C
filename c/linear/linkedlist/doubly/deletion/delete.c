...
struct node *doublyDeleteAtIndex(struct node *start, int index)
{
    ...
    verboseflag == 1 ? doubly_display_node(p, "STEP (p): Check if the node is the last node") : "";
    if (p->nextpointer != NULL)
    {
        p->previouspointer->nextpointer = p->nextpointer;
        verboseflag == 1 ? doubly_display_node(p->previouspointer, "STEP (p->previouspointer): Set the next pointer of the P previous pointer to P next pointer") : "";

        p->nextpointer->previouspointer = p->previouspointer;
        verboseflag == 1 ? doubly_display_node(p->nextpointer, "STEP (p->nextpointer): Set the previous pointer of the P next pointer to P previous pointer") : "";
    }
    else
    {
        p->previouspointer->nextpointer = NULL;
        verboseflag == 1 ? doubly_display_node(p->previouspointer, "STEP (p->previouspointer): If yes, Set the previous pointer of the P next pointer to NULL") : "";
    }

    free(p);
    verboseflag == 1 ? doubly_display_node(p, "STEP (p): Free the 'to be deleted node' from memory") : "";
...
struct node *doublyDeleteElement(struct node *start, int element)
{
    ...
    p->previouspointer->nextpointer = p->nextpointer;
    verboseflag == 1 ? doubly_display_node(p->previouspointer, "STEP (p->previouspointer): Set the next pointer of the node previos to p as the p next pointer") : "";

    verboseflag == 1 ? doubly_display_node(p, "STEP (p): Check if p is the last node") : "";
    if (p->nextpointer != NULL)
    {
        p->nextpointer->previouspointer = p->previouspointer;
        verboseflag == 1 ? doubly_display_node(p->nextpointer, "STEP (p->nextpointer): If no, set the previous pointer of the next pointer to p previous node") : "";
    }
    else
    {
        p->previouspointer->nextpointer = NULL;
        verboseflag == 1 ? doubly_display_node(p->previouspointer, "STEP (p->previouspointer): If yes set the next pointer of the previous pointer as NULL") : "";
    }
