...
struct node *doublyDeleteAtIndex(struct node *start, int index)
{
    ...
    if (index == 0)
    {
        ...
        start = p;
        verboseflag == 1 ? doubly_display_node(p, "STEP 6: Replace new start node as node at pointer p") : "";
    ...
    verboseflag == 1 ? doubly_display_node(temp, "STEP 5: TEMP : The node to which the to be deleted node is pointing is saved to a temp node") : "";

    free(p->pointer);
    verboseflag == 1 ? doubly_display_node(p, "STEP 6: Free the 'to be deleted node' from memory") : "";

    p->pointer = temp;
    verboseflag == 1 ? doubly_display_node(p, "STEP 7: Point p node pointer to the temp pointer that we saved earlier") : "";
...
struct node *doublyDeleteElement(struct node *start, int element)
{
    ...
    p->pointer = temp;
    verboseflag == 1 ? doubly_display_node(p, "STEP 5: Point the p node pointer to the temp node") : "";
