...
struct node *doubly_create_linked_list(struct node *start, int length)
{
    ...
    for (int i = 0; i < length; i++)
    {
        ...
        verboseflag == 1 ? doubly_display_linked_list(temp, "STEP : Initialize memory for a temp node (payload + pointer) memory") : "";

        temp->payload = user_input;
        verboseflag == 1 ? doubly_display_linked_list(temp, "STEP : Set the payload value in temp") : "";

        temp->pointer = p;
        verboseflag == 1 ? doubly_display_linked_list(temp, "STEP : Set temp pointer to p node") : "";
